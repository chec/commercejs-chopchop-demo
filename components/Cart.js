...
export default function Cart() {
  ...
  const { hideCart } = useCartDispatch();

  if (!open) return null;

  return (
    <motion.div
      className="bg-ecru-white z-50 h-full overflow-scroll fixed inset-0"
    ...
    >
      <div className="h-full container mx-auto px-3 md:px-4 lg:px-5 flex flex-col justify-between">
        ...
        <div className="flex items-center justify-between py-3 md:py-4 lg:py-5">
          ...
          <div>
            <Button className="appearance-none leading-none p-1 md:p-2 lg:p-3 text-lg md:text-xl">
              Check Out
            </Button>
          </div>
        </div>
